...
if ls submodule/vcpkg/vcpkg* 1> /dev/null 2>&1; then
    ...
    if [ "${triplet}" == "x86-windows" ] || [ "${triplet}" == "x64-windows" ]; then
        ...
        X:/vcpkg/vcpkg install "eigen3:${triplet}" "libigl:${triplet}"
        ...
        # install/build dependencied
        submodule/vcpkg/vcpkg install "eigen3:${triplet}" "libigl:${triplet}"
    fi
fi
...
mkdir -p build
cd build
cmake .. -DCMAKE_TOOLCHAIN_FILE="submodule/vcpkg/scripts/buildsystems/vcpkg.cmake" -DVCPKG_TARGET_TRIPLET="${triplet}"
